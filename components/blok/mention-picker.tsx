...
export function MentionPicker({ onSelect, onClose }: MentionPickerProps) {
  ...
  const { activeGroupId, members } = useGroupStore();
  const [searchQuery, setSearchQuery] = useState('');

  // Get all mentionable users (friends + group members)
  const mentionableUsers = friends.map(f => f.friend).filter(Boolean);
    ...
    <div className="absolute bottom-full right-0 mb-2 w-64 bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg shadow-xl overflow-hidden animate-slide-in">
      ...
      <div className="max-h-48 overflow-y-auto p-1">
        {filteredUsers.length === 0 ? (
          <p className="p-2 text-sm text-[var(--text-muted)] text-center">{t("mention.noUsersFound")}</p>
        ) : (
