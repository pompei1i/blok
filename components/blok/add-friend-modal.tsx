...
import { X, UserPlus, Search, Check, Clock, Send } from 'lucide-react';
...
export function AddFriendModal({ isOpen, onClose }: AddFriendModalProps) {
  const { t } = useI18n();
  const { sendFriendRequest, outgoingRequests, cancelRequest } = useFriendsStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = sendFriendRequest(username.trim());
    setMessage({ 
      ...
      <div className="relative w-full max-w-md bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg shadow-2xl animate-fade-in">
        ...
        <div className="flex items-center justify-between p-4 border-b border-[var(--border)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-[var(--text-primary)]">{t("addFriend.title")}</h2>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {isSubmitting ? t("addFriend.sending") : t("addFriend.send")}
              </button>
            </div>
          </form>
                ...
                {outgoingRequests.map((request) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <UserAvatar user={request.friend} size="sm" />
