...
const EXPIRY_LABELS: Record<ExpiryOption, string> = { "1d": "1 day", "7d": "7 days", never: "∞" };
...
export function InviteUserModal({ isOpen, onClose, serverId, serverName }: InviteUserModalProps) {
  ...
  const usesLabel =
    server?.inviteMaxUses != null
      ? `${server.inviteUsedCount ?? 0} / ${server.inviteMaxUses} uses`
      : null;
            ...
            <>
              <div className="space-y-2">
                ...
                {inviteCode ? (
                  ...
                  <p className="text-xs text-[var(--text-muted)] px-1">No invite code yet.</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                <label className="flex items-center gap-1 text-xs text-[var(--text-muted)] uppercase tracking-wider">
                  <Clock className="w-3 h-3" /> Expires after
                </label>
