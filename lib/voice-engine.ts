...
export class VoiceEngine {
  ...
  setInputVolume(volume: number): void {
    // Volume applied via gain node if wired up
    if (this.outputMasterGain) {
      this.outputMasterGain.gain.value = volume / 100;
    }
  }
  ...
  async startScreenShare(): Promise<void> {
    ...
    if (!videoTrack) { this.screenStream = null; return; }
    ...
    for (const pc of this.peers.values()) {
      if (pc.signalingState !== 'closed') {
        pc.getSenders()
          ...
          .forEach((s) => { try { pc.removeTrack(s); } catch { /* ignore */ } });
      }
    }
  ...
  // Force Opus fullband (48 kHz) mode to avoid radio-sounding voice.
  ...
  private getOrCreatePeer(peerId: string): RTCPeerConnection {
    ...
    pc.onicecandidate = ({ candidate }) => {
      if (candidate) {
        void this.broadcast({ type: 'ice', from: this.userId, to: peerId, candidate: candidate.toJSON() });
      }
    };

    pc.onnegotiationneeded = async () => {
      if (pc.signalingState === 'stable' && this.peers.get(peerId) === pc) {
        try { await this.createOffer(peerId); } catch { /* ignore race */ }
      }
    };
  ...
  private closePeer(peerId: string): void {
    ...
    if (t) { clearInterval(t); this.speakingTimers.delete(peerId); }
