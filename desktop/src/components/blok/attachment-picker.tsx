...
const attachmentTypes = [
  { icon: Image, labelKey: "attachment.image", accept: "image/*", color: "text-green-400" },
  { icon: Film, labelKey: "attachment.video", accept: "video/*", color: "text-purple-400" },
  {
    icon: FileText,
  ...
  { icon: Mic, labelKey: "attachment.audio", accept: "audio/*", color: "text-yellow-400" },
] as const;
  ...
  return (
    <div className="absolute bottom-full left-0 mb-2 w-72 bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg shadow-xl overflow-hidden animate-slide-in">
      ...
      <div className="flex items-center justify-between p-2 border-b border-[var(--border)]">
        ...
        <button
          onClick={onClose}
          className="p-1 hover:bg-[var(--bg-hover)] rounded transition-colors text-[var(--text-muted)]"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="grid grid-cols-4 gap-1 p-2 border-t border-[var(--border)]">
        {attachmentTypes.map(({ icon: Icon, labelKey, accept, color }) => (
          <button
            key={labelKey}
          ...
          >
            ...
            <span className="text-xs text-[var(--text-muted)]">{t(labelKey)}</span>
          </button>
        ))}
      </div>
    </div>
